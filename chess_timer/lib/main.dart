              ...
              builder: (context, state) => Column(
                    children: <Widget>[
                        ...
                        child: Transform.rotate(
                          ...
                          child: Container(
                            ...
                            child: Transform.scale(
                              scale: state.playerScale[1],
                              child: PlayersArea(
                                isActive: state.playerAtTurn == 1,
                                time: state.playerTime[1],
                                clickedCallback: () => BlocProvider.of<ChessTimerBloc>(context)
                      ...
                      MiddleArea(
                        ...
                        () => BlocProvider.of<ChessTimerBloc>(context).dispatch(ResetEvent()),
                      ),
                      Expanded(
                        child: Container(
                          margin: EdgeInsets.all(8),
                          child: Transform.scale(
                            scale: state.playerScale[2],
                            child: PlayersArea(
      ...
      ..addListener(() {
        setState(() {
          _playerScale.asMap().forEach((index, value) {
            _playerScale[index] =
                index == BlocProvider.of<ChessTimerBloc>(context).currentState.playerAtTurn ? animation.value : 1.0;
          });
        });
      });
    _animationSubscription = BlocProvider.of<ChessTimerBloc>(context).animationStream.listen((_) => _startAnimation());
  }
